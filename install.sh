...
npm list -g                | grep -q 'yarn'   || sudo npm i -g yarn
npm list -g                | grep -q 'neovim' || sudo npm i -g neovim
gem list                   | grep -q 'neovim' || gem install neovim
echo "python3 -m pip list" | grep -q 'pynvim' || python3 -m pip install pynvim
